...
std::vector<std::vector<Coord>> subdividePolygon(const std::vector<Coord>& perimeter, const std::vector<Edge>& diagonals) {
    ...
    for (const Edge& diagonal : sortedDiagonals) {
        if (diagonalUsed[diagonal.start.index] || diagonalUsed[diagonal.end.index]) {
            // This diagonal's endpoint(s) are already used in previous subPolygons
            continue;
        }

        diagonalUsed[diagonal.start.index] = true;
        diagonalUsed[diagonal.end.index] = true;

        std::vector<Coord> subPolygon;
        subPolygon.push_back(diagonal.start);

        Coord current = diagonal.end;
        bool diagonalTaken = false;
        while (current.index != diagonal.start.index) {
            subPolygon.push_back(current);
            if(!diagonalTaken)
                for (const Edge& otherDiagonal : diagonals) {
                    if (otherDiagonal.start.index == current.index && diagonal.start.index != current.index) {
                        current = otherDiagonal.end;
                        diagonalTaken = true;
                        break;
                    } else if (otherDiagonal.end.index == current.index && diagonal.end.index != current.index) {
                        current = otherDiagonal.start;
                        diagonalTaken = true;
                        break;
                    }
                }
    ...
    // Add the remaining vertices to a single polygon
    std::vector<Coord> remainingPolygon;
    for (int i = 0; i < numVertices; ++i) {
        if (!diagonalUsed[i]) {
            bool diagonalTaken = false;
            Coord current = perimeter[(i+1)%perimeter.size()];
            remainingPolygon.push_back(current);
            while (current.index != i) {
                if(!diagonalTaken)
                    for (const Edge& otherDiagonal : diagonals) {
                        if (otherDiagonal.start.index == current.index ) {
                            current = otherDiagonal.end;
                            diagonalTaken = true;
                            break;
                        } else if (otherDiagonal.end.index == current.index) {
                            current = otherDiagonal.start;
                            diagonalTaken = true;
                            break;
                        }
    ...
    subPolygons.push_back(remainingPolygon);
